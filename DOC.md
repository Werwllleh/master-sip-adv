...
npx gulp
```
Понять логику работы проще всего на реальном примере.
### Создание страницы
выполнить в командной строке:
```
node cli/createPage
```
Вводим название страницы (например contacts), и в папке pages у нас появляется одноименная папка с файлами contacts.pug, contacts.scss, contacts.js
...
```
Вводим название блока (например section-slider), и в папке blocks у нас появляется одноименная папка с файлами section-slider.pug, section-slider.scss, section-slider.js
...
### Картинки, шрифты ...
Все что сохранено в папке public копируется в dist

### SVG спрайты
автоматическая генерация еще не готова, так что сваливаем все иконки в src/sprite/src, чистим, если нужно цвета меняем на currentColor, идем на https://svgsprit.es/ и перетаскиваем весь src в генератор. Затем сохраняем полученное в sprite/sprite.svg, вставляем иконку через миксин +svg('icon-name')
Для примера смотреть кнопку закрыть в модалке

### SCSS
Пока не настроены глобальный проброс переменный в файлах, где нужны данные из app/scss/variables в каждом файле приходится подключать их вручную:
```
...
### Что в итоге
Все файлы стилей компилируются в один файл style.css.
Все файлы скриптов склеиваются в единый файл common.js. Весь код оборачивается в событие DOMContentLoaded, поэтому в режиме разработки в блоках/компонентах/скриптов страниц нет необходимости это делать (главное не забывать вешать проверки).

### Почему всё клеится в один файл стилей и в один файл скриптов?
Товарищам битриксоидам так удобнее, чем когда всё раздроблено на блоки, компоненты или отдельные файлы стилей и скриптов у страниц. Всё-таки Bitrix, а не React или Vue.
